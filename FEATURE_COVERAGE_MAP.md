...
| Reliability | PARTIAL | Reliability history/score/trend/prediction/pattern APIs now have real local-database runtime paths, dump/exception/stack-trace flows now have a real local-filesystem path, and durable `ReliabilityRun` history plus read/detail APIs now exist, but deeper investigation workflows are still incomplete. | `server/services/reliability_service.py`, `tests/test_reliability_api.py`, `tests/test_phase6_reliability_operator.py` |
